...
export function buildLinks(sharedParams, urlToFile) {
	if (sharedParams.type.includes("WBNRS")) {
		...
		const correctUrlLink =
			urlToFile + sharedParams.furriel +
			sharedParams.kw +
			sharedParams.matomo +
			sharedParams.matomo_medium +
			sharedParams.matomo_cid;

		return [correctFinalLink, correctPixel, correctBannerLink, correctButtonLink, correctUrlLink]
	} else {
		...
		return [correctSede, correctFinalLink, correctPixel, correctBannerLink, correctButtonLink, correctUrlLink]
	}
